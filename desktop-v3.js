...
(()=>{
 ...
 function navigate(step){if(busy||arrows.hidden||document.getElementById('directoryDialog').open)return;const entries=order(),index=entries.indexOf(current);if(index<0)return;travel(entries[(index+step+entries.length)%entries.length],document.querySelector('.scene.active'),false,step)}
